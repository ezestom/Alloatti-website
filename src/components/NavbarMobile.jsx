...
export function NavbarMobile() {
	...
	const navItems = [
		...
		},
		{
			...
			activeIcon: BsQuestionSquareFill,
		},
		{
			to: "/normas-de-calidad",
			label: "Ventas",
			icon: GiNetworkBars,
			activeIcon: GiNetworkBars,
		},
		{
			to: "/contacto",
			label: "Contacto",
	...
	const isSalesPath = ["/normas-de-calidad", "/ultimas-noticias"].includes(
		location.pathname,
	);
	...
	return (
		<div className="z-30 fixed bottom-0 w-full px-4 pb-4 flex flex-col gap-2">
			...
			{(isMachinesPath || isSalesPath) && (
				<div
					className={`flex overflow-x-auto justify-evenly scrollbar-hide py-2 px-2 gap-2 rounded-xl border shadow-sm backdrop-blur-sm transition-all duration-500 ${
						isDarkTheme
							? "bg-[#292929]/80 border-slate-700"
						...
						return (
							<li key={item.to} className="relative flex-1">
								<NavLink
									onClick={scrollToTop}
									to={item.to}
									className={`flex w-full flex-col items-center justify-center gap-1.5 transition-all duration-300 ${
										isActive
											? isDarkTheme
												? "text-blue-400"
												: "text-[#024ad8]"
									...
									<span
										className={`text-[8px] font-extrabold uppercase tracking-wider transition-opacity duration-300 ${isActive ? "opacity-100" : "opacity-70"}`}>
										{item.label}
									</span>
									{isActive && (
										<span
											className={`absolute -bottom-3 w-1 h-1 rounded-full ${isDarkTheme ? "bg-blue-400" : "bg-[#024ad8]"} shadow-lg shadow-blue-500/50`}></span>
									)}
								</NavLink>
							</li>
						);
