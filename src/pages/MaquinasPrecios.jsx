		...
		<div className="print-card bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-3xl shadow-lg p-6 min-w-[300px] space-y-6">
						...
						<div key={item.id} className="py-3 flex justify-between items-center gap-4 text-xs">
							...
							<div className="p-2.5 bg-slate-100 dark:bg-zinc-800 hover:bg-slate-200 dark:hover:bg-zinc-700/80 text-slate-700 dark:text-slate-200 rounded-xl font-bold text-xs uppercase transition-all flex items-center justify-center gap-1.5 border border-slate-200 dark:border-zinc-700">
								...
								<button 
									...
									className="no-print text-red-500 hover:text-red-700 w-5 h-5 rounded hover:bg-red-50 dark:hover:bg-red-950/20 flex items-center justify-center">
			...
			<div className="border-t border-slate-100 dark:border-zinc-800 pt-4 space-y-3">
				...
				<div className="flex justify-between text-xs">
					...
					<span className="font-bold text-slate-800 dark:text-slate-150">
						{formatValue(showIva ? subtotalDisplay - (subtotalDisplay/1.105) : subtotalDisplay * 0.105)}
					</span>
				</div>
				...
				<div className="flex justify-between items-center text-sm border-t border-dashed border-slate-100 dark:border-zinc-800 pt-3">
					<span className="text-slate-800 dark:text-white font-extrabold">Total Estimado:</span>
...
export function MaquinasPrecios() {
	...
	const subtotalDisplay = getDisplayPrice(subtotalUSD);
	const totalDisplay = showIva ? subtotalDisplay : subtotalDisplay * 1.105;
	...
	const handleWhatsAppShare = () => {
		...
		const details = `Hola Alloatti, me interesa cotizar los siguientes equipos:\n\n${productsText}\n\n*Resumen Estimado (${currency}):*\n- Subtotal Neto: ${formatValue(showIva ? subtotalDisplay / 1.105 : subtotalDisplay)}\n- IVA (10.5%): ${formatValue(showIva ? subtotalDisplay - (subtotalDisplay/1.105) : subtotalDisplay * 0.105)}\n- *Total Estimado: ${formatValue(totalDisplay)}*`;
		
		const url = `https://wa.me/5493415033878?text=${encodeURIComponent(details)}`;
	...
	return (
		<>
			...
			<div className="interactive-layout">
				...
				{/* Share Toast */}
				{showShareToast && (
					<div className="fixed bottom-6 right-6 z-50 bg-[#024ad8] text-white px-5 py-3 rounded-xl shadow-xl flex items-center gap-3 animate-bounce no-print">
						<svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
							<path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
						</svg>
						<span className="text-sm font-semibold">Enlace de cotización copiado al portapapeles!</span>
					</div>
				)}
					...
					<div className="flex-1">
						...
						<div className="no-print mt-8 p-6 bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-2xl shadow-sm space-y-6">
							<div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-center">
								...
								<div>
									...
									<div className="flex bg-slate-100 dark:bg-zinc-800 p-1 rounded-xl">
										<button 
											...
											className={`flex-1 py-2 text-xs font-bold rounded-lg transition-all ${currency === "USD" ? "bg-white dark:bg-zinc-700 text-[#024ad8] dark:text-white shadow-sm" : "text-slate-500 hover:text-slate-800 dark:hover:text-slate-200"}`}>
											DÓLARES (USD)
										</button>
										<button 
								...
								{/* BNA Exchange Input */}
								<div className={currency === "ARS" ? "opacity-100 transition-all duration-300" : "opacity-40 pointer-events-none transition-all duration-300"}>
										...
										<input 
											...
											className={`w-full border rounded-xl py-2 pl-8 pr-4 text-sm font-semibold focus:outline-none focus:border-[#024ad8] ${
												(!isManualExchangeRate && !apiFailed) 
													? "bg-slate-100 dark:bg-zinc-800 text-slate-500 dark:text-zinc-400 border-slate-200 dark:border-zinc-700 cursor-not-allowed" 
									...
									{currency === "ARS" && (
										<div className="mt-1 text-[10px] font-medium min-h-[15px] text-left">
											{apiLoading ? (
												...
												<span className="text-red-500">Error de conexión. Ingrese el tipo de cambio manual.</span>
											) : !isManualExchangeRate ? (
												...
												<span className="text-amber-500">Modo de edición manual activado.</span>
											)}
										</div>
									)}
								</div>
															...
															{qty > 0 ? (
																<div className="flex items-center bg-slate-100 dark:bg-zinc-800 rounded-xl p-1 border border-slate-200 dark:border-zinc-700">
																	<button 
																		...
																		className="w-7 h-7 rounded-lg bg-white dark:bg-zinc-700 hover:bg-slate-200 text-slate-650 flex items-center justify-center font-bold text-base shadow-sm">
																		-
																	</button>
